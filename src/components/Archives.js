...
const Archives = ({ onFolderSelect, currentFolderId }) => {
  ...
  const fetchFiles = useCallback(() => {
    let query = db.collection("myfiles").where("category", "==", "archive");
  ...
  const handleFolderClick = (folderId, folderName, isProtected) => {
    if (isProtected) {
      ...
      onFolderSelect('ARCHIVES', folderId, folderName, newFolderPath);
    }
  };
    ...
    async (acceptedFiles) => {
      ...
      try {
        const uploadPromises = acceptedFiles.map(async (file) => {
          ...
          const storageRef = storage.ref(`archive/${file.name}`);
          const uploadTask = storageRef.put(file);
  ...
  const handlePasswordSubmit = () => {
    ...
    if (folder && folder.password === password) {
      ...
      onFolderSelect('ARCHIVES', selectedFolder.id, selectedFolder.name, newFolderPath);
  ...
  const handlePasswordVerified = () => {
    if (selectedProtectedFolder) {
      ...
      onFolderSelect('ARCHIVES', selectedProtectedFolder.id, selectedProtectedFolder.filename, newFolderPath);
