...
const Brand = ({ onFolderSelect, currentFolderId }) => {
    ...
    async (acceptedFiles) => {
      ...
      try {
        const uploadPromises = acceptedFiles.map(async (file) => {
          ...
          const storageRef = storage.ref(`brand/${file.name}`);
          const uploadTask = storageRef.put(file);
  ...
  const handlePasswordSubmit = () => {
    ...
    if (folder && folder.password === password) {
      ...
      onFolderSelect('BRANDS', selectedFolder.id, selectedFolder.name, newFolderPath);
  ...
  const handlePasswordVerified = () => {
    if (selectedProtectedFolder) {
      ...
      onFolderSelect('BRANDS', selectedProtectedFolder.id, selectedProtectedFolder.filename, newFolderPath);
